...
class CGallery {
     ...
     public function readAllItemsInDir($path, $validImages = array('png', 'jpg', 'jpeg')) {
        ...
        foreach($files as $file) {
            ...
            if(is_file($file) && in_array($parts['extension'], $validImages)) {
                $item    = "<img src='{$this->imgPath}img.php?src=" . GALLERY_BASEURL . $href . "&amp;width=128&amp;height=128&amp;crop-to-fit&amp;=sharpen' alt=''/>";
                $caption = basename($file);
            }
            elseif(is_dir($file)) {
                $item    = "<img src='img/folder.png' alt='folder'/>";
                $caption = basename($file) . '/';
            }
            else {
                continue;
     ...
     */
     public function readItem($path, $validImages = array('png', 'jpg', 'jpeg')) {
        ...
        $item = <<<EOD
<p><img class='big-picture' src='{$this->imgPath}img.php?src={$href}{$displayWidth}{$displayHeight}&amp;=sharpen' alt='{$href}'/></p>
<p>Original image dimensions are {$width}x{$height} pixels. <a href='{$this->imgPath}img.php?src={$href}'>View original image</a>.</p>
